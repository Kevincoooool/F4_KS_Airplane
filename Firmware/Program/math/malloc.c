...
{  
    mymemset(mallco_dev.memmap[0], 0,memtblsize[0]*2);//内存状态表数据清零  
	mymemset(mallco_dev.membase[0], 0,memsize[0]);	//内存池所有数据清零  
...
//    else  
//    {  									   
//	    mymemcpy((void*)((u32)mallco_dev.membase[0]+offset),ptr,size);	//拷贝旧内存内容到新内存   
//        myfree(ptr);  											  		//释放旧内存
