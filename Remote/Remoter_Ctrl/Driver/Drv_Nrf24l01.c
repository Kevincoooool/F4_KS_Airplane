/******************** (C) COPYRIGHT 2014 KS Tech ***************************
 * 作者		 ：匿名科创
 * 文件名  ：Drv_Nrf24l01.cpp
 * 描述    ：Nrf24l01
 * 官网    ：www.KStc.com
 * 淘宝    ：KStc.taobao.com
 * 技术Q群 ：190169595
...
void NRF_Check_Ch(void)
{
	if(Check_Ch)
	{
		...
		{
			NRF_Write_Reg(NRF_WRITE_REG+RF_CH,Param.NRF_Channel++);
			if(Param.NRF_Channel>125) Param.NRF_Channel = 0;
		}
	}
}
...
void NRF_Check_Event(void)
{
	...
	if(sta & (1<<RX_DR))
	{
		...
		if(RX_LEN<33)
		{
			...
			DT_NrfData_Anl(NRF24L01_2_RXDATA , RX_LEN);
			NRF_SSI_CNT++;
			NRF_Evnet = 1;
		}
		else 
