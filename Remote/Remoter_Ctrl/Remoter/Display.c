...
void show_state(void)
{
	...
	switch(refresh_line)//显示遥控状态界面
	{
		...
		case 1:
			...
			OLED_ShowNum(29,2,STICK_ROL,4,6);//显示ASCII字符的码值
			OLED_ShowNum(95,2,STICK_PIT,4,6);//显示ASCII字符的码值
			OLED_ShowString(0,3,"THR:       YAW:    ",6); 
			OLED_ShowNum(29,3,STICK_THR,4,6);//显示ASCII字符的码值	
			OLED_ShowNum(95,3,STICK_YAW,4,6);//显示ASCII字符的码值
			break;
		case 2:
...
void Display_30ms(void)
{
	if(Menu_State == 0)						//显示正常界面
	{
		if(KEY_VALUE==11)				//长按BUT
		{
			Menu_State = 2;						//显示摇杆校准界面
			OLED_Clear();
			KEY_VALUE = 0;
		}
		else if(KEY_VALUE==1)		//短按BUT
		{
			Menu_State = 1;					//进入菜单界面
			OLED_Clear();
			KEY_VALUE = 0;
		}
		else if(KEY_VALUE==12)	//长按右摇杆
		{
			KEY_VALUE = 0;
			Stick_Mode = !Stick_Mode;//切换控制模式
		}
		else if(KEY_VALUE==2)		//短按右摇杆
		{
			KEY_VALUE = 0;
			Stick_Mode = !Stick_Mode;//切换控制模式
		}
